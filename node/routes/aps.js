...
router.delete('/aps/:id', async (req, res) => {
    ...
    if (idx === -1) {
        return res.status(404).json({status: 'error', message: 'Точка доступа не найдена'})
    }
    aps.splice(idx, 1)
    try {
        await saveAps()
        res.status(200).json({status: 'success', message: 'Точка доступа успешно удалена', id: req.params.id})
    } catch (err) {
