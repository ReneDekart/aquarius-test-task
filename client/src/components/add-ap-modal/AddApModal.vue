<!--
  Модальное окно добавления точки доступа.

  Компонент выводит диалог el-dialog билиотеки Element UI с формой для ввода данных
  новой точки доступа: название, модель, IP-адрес, диапазон частот (band),
  канал и местоположение.

  Логика работы:
  - видимость формы управляется из родительского компонента через v-model (porp "value")
  - при нажатии на кнопку добавления вызывается метод onApplyClick
  - при нажатии на кнопку отмену вызывается метод onCloseClick

  События компонента формы:
  - input (возвращает Boolean) — обновление видимости окна для v-model;
  - ap-created (возвращает Object) — данные созданной точки доступа (name, model, ip,
    band, channel, location).
-->
<template>
  <el-dialog
...
<script>
/**
 * getValidations - функция для получения настроек валидации для полей формы
 * getPlaceholders - функция для получения плейсхолдеров с переводом
 */
